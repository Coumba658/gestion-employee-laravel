            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">
                        {{ __('Modification Pays') }}
                        <a href="{{ route('countries.index') }}" class="float-right">Retour</a>
                    </div>
                        ...
                        <form method="POST" action="{{ route('countries.update', $country->id) }}">
                            ...
                            <div class="form-group row">
                                <label for="country_code"
                                    class="col-md-4 col-form-label text-md-right">{{ __('Code pays') }}</label>
                ...
                <div class="m-2 p-2">
                    <form method="POST" action="{{ route('countries.destroy', $country->id) }}">
                        @csrf
                        @method('DELETE')
                        <button class="btn btn-danger">Supprimer {{ $country->country_code }}</button>
                    </form>
                </div>
            </div>
        </div>
